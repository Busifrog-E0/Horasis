...
const EditEvent = () => {
	...
	const [eventId, setEventId] = useState(eventid ? eventid : '')
	...
	const getEvent = () => {
		getItem(
			...
			(result) => {
				setEvent(result)
				setPostEventData((prev) => ({ ...prev, ...result }))
			},
			(err) => {},
			updateCurrentUser,
			currentUserData,
	...
	const onDisplayImageSelect = (imageData) => {
		...
		setPostEventData((prev) => ({ ...prev, DisplayPicture: tempUrl }))
	}
	const onDisplayImageDelete = () => {
		setDisplayImageToUpload(null)
		setPostEventData((prev) => {
			const { DisplayPicture, ...rest } = prev
			return rest
		})
	}
	...
	const onCoverImageSelect = (imageData) => {
		...
		const tempUrl = URL.createObjectURL(new Blob([new Uint8Array(imageData.FileData)]))
		setPostEventData((prev) => ({ ...prev, CoverPicture: tempUrl }))
	}
	const onCoverImageDelete = () => {
		setCoverImageToUpload(null)
		setPostEventData((prev) => {
			const { CoverPicture, ...rest } = prev
			return rest
		})
	}
	...
	const onDisplayImageUpload = (coverImageUrl) => {
		if (displayImageToUpload) {
			...
			onImageSet(coverImageUrl, postEventData?.DisplayPicture)
		}
	}
	...
	const onCoverImagesUpload = (imageToUpload) => {
		if (imageToUpload) {
			...
			onDisplayImageUpload(postEventData?.CoverPicture)
		}
	}
			...
			<div className='lg:col-span-2'>
				...
				<div className='p-6 bg-system-secondary-bg rounded-lg relative overflow-hidden'>
					...
					{activeStep === 3 && (
						<CreateEventStep3
							ref={step3Ref}
							selectedImage={postEventData?.DisplayPicture}
					...
					{activeStep === 4 && (
						<CreateEventStep4
							ref={step4Ref}
							selectedImage={postEventData?.CoverPicture}
