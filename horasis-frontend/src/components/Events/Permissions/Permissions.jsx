...
const Permissions = ({ permissionType, eventId, event, from = 'settings' }) => {
	...
	const api = `events/${eventId}/members`
	...
	const removeUserPermission = (member) => {
		setIsRemoving(true)
		patchItem(
			`events/${eventId}/member/${member.UserDetails.DocId}/permissions/remove`,
			{
	...
	const [permitEveryone, setPermitEveryone] = useState(() => {
		if (from === 'settings') {
			return event.MemberPermissions[`${permissionType}`]
		} else {
			return false
		}
	})
	...
	const permissionForEveryone = () => {
		patchItem(
			`events/${eventId}/member/permissions/everyone`,
			{
