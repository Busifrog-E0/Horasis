...
const MyConnectionsTab = ({ showOther = false, userId = '' }) => {
	...
	const UserId = showOther === true ? userId : currentUserData.CurrentUser.UserId
	const getConnections = (tempConnections) => {
		setLoadingCom(tempConnections, true)
		getItem(
			`users/${UserId}/connections?&NextId=${getNextId(tempConnections)}&${jsonToQuery(filters)}`,
			(data) => {
	...
	const hasAnyLeft = () => {
		getItem(
			`users/${UserId}/connections?NextId=${getNextId(connections)}&${jsonToQuery({
				...filters,
				Limit: 1,
			})}`,
			(data) => {
