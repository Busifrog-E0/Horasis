...
import usePostDataSuperadmin from '../../hooks/usePostDataSuperadmin'
...
const PlatformUsers = () => {
	...
	const { data: users, isLoading, isLoadingMore, isPageDisabled, getList: getUsers } = useGetListSuperadmin('users')

	const { postData: addUser, isLoading: isAddingUser } = usePostDataSuperadmin()
...
const AddPlatformUserModal = ({ isAddingUser, addUserModalOpen, setAddUserModalOpen, addNewUser = () => {} }) => {
	...
	const { postData: checkUsername } = usePostDataSuperadmin({
		onSuccess: (result) => {
			if (result === true) {
