...
const SidebarTab = ({ tabs, title = "User Analytics" }) => {
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-4 lg:gap-16">
      <div>
        {title &&
          <h4 className="font-medium text-base text-brand-gray-dim mt-3 lg:mt-5 mb-3 lg:mb-5 mx-2">{title}</h4>
        }
        <TabList
      ...
      {tablist.map((item, index) => {
        return (
          <div onClick={() => {
            ...
            className={`p-4 py-3 cursor-pointer font-medium flex flex-row gap-3 items-center ${activeTab === index
