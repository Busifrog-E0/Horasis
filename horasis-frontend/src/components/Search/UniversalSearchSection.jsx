...
const UniversalSearchSection = () => {
    ...
    const getPosts = (tempPosts) => {
        getData(
            `${'activities'}?${jsonToQuery(filters)}`,
            tempPosts,
            setPosts
        )
    }
    const getDiscussions = (tempDiscussions) => {
        getData(
            `discussions?${jsonToQuery(filters)}`,
            tempDiscussions,
            setDiscussions
        )
    }
    ...
    useEffect(() => {
        switch (activeTab) {
            ...
            case 2:
                if (posts.length > 0)
                    hasAnyLeft(`${'activities'}`, posts)
                break
            case 3:
                if (events.length > 0)
                    hasAnyLeft(`events`, events)
                break
            case 4:
