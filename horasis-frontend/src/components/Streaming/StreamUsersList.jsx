...
const StreamUsersList = ({ event, localMicrophoneTrack, cameraOn, micOn, localCameraTrack, setCamera, remoteUsers, isConnected, calling, setCalling, setMic, role, currentUser }) => {
	return (
		<div className='flex-1'>
			<div className='flex flex-row gap-4 p-6 items-center border-b border-system-secondary-bg-transparent'>
			...
			<div className='overflow-auto h-full mx-auto'>
				<div className='overflow-hidden rounded-lg'>
					<div className=' grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 p-4'>
						{role === 'Speaker' && (
							<div className=' h-96 flex flex-col items-center rounded-xl overflow-hidden'>
								<LocalUser audioTrack={localMicrophoneTrack} cameraOn={cameraOn} micOn={micOn} videoTrack={localCameraTrack} cover={avatar} className='w-32 h-32 bg-orange-500 relative'>
									...
									<span className='absolute right-0  bottom-0 font-semibold text-brand-secondary bg-system-primary-accent px-3 m-2 rounded-full'>
										{'You'} {micOn ? <img className='inline-block h-4' src={mic}></img> : <img src={mic_off} className='inline-block h-4'></img>}
									</span>
								</LocalUser>
							</div>
						)}
						{remoteUsers.map((user) => (
							<div className=' h-96 flex flex-col items-center rounded-xl overflow-hidden' key={user.uid}>
								<RemoteUser cover={avatar} user={user} className=' w-32 h-32 bg-red-500'>
									<div className='absolute right-0 rounded-full m-2 bottom-0 font-semibold text-brand-secondary bg-system-primary-accent px-3'>
										{user.uid} {user.hasAudio ? <img className='inline-block h-4' src={mic}></img> : <img src={mic_off} className='inline-block h-4'></img>}
									</div>
								</RemoteUser>
							</div>
						))}
						...
						{isConnected && (
							<div className='flex justify-center items-center'>
								<div className='flex space-x-5 justify-center items-center'>
									...
									<button className={`btn btn-phone p-4 rounded-lg ${calling ? 'bg-red-500' : 'bg-green-500'} text-white`} onClick={() => setCalling((a) => !a)}>
										{calling ? <img src={call_end}></img> : <img src={call_end}></img>}
									</button>
									<button className='bg-brand-btn-prim p-2 rounded-full hover:bg-brand-seagreen-dim' onClick={() => setMic((a) => !a)}>
										{micOn ? <img src={mic}></img> : <img src={mic_off}></img>}
