...
const StreamUsersList = ({ event, localMicrophoneTrack, cameraOn, micOn, localCameraTrack, setCamera, remoteUsers, isConnected, calling, setCalling, setMic, role }) => {
	return (
					...
					<div className=' grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 p-4'>
						{role === 'Speaker' && (
							<div className=' h-96 flex flex-col items-center'>
								<LocalUser audioTrack={localMicrophoneTrack} cameraOn={cameraOn} micOn={micOn} videoTrack={localCameraTrack} cover={testImage} className='w-32 h-32 bg-orange-500 relative'>
						...
						{remoteUsers.map((user) => (
							<div className=' h-96 flex flex-col items-center' key={user.uid}>
								<RemoteUser cover={testImage} user={user} className=' w-32 h-32 bg-red-500'>
