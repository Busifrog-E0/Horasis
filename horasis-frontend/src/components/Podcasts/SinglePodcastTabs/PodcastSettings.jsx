...
const PodcastSettings = ({ podcastId, from = 'settings', podcast }) => {
	...
	const getAllMembers = (tempMembers) => {
		getData(`podcasts/${podcastId}/members?&${jsonToQuery(filters)}`, tempMembers, setMembers)
	}
	...
	useEffect(() => {
		if (members.length > 0) hasAnyLeft(`podcasts/${podcastId}/members`, members)
	}, [members])
	...
	const patchSavePermissions = (patchData) => {
		patchItem(
			`podcasts/${podcastId}/member/permissions`,
			patchData,
			(result) => {
				console.log(result)
			},
	...
	const viewPodcastPage = () => navigate(`/Podcasts/${podcastId}`)
	...
	return (
		<>
			<Modal isOpen={deleteModal}>
				<Modal.Header>
					<p className='text-lg font-medium'>You are about delete the podcast</p>
					<button
						onClick={() => {
				...
				<Modal.Body>
					<div className='flex items-center justify-end gap-2'>
						...
						<Button variant='danger' onClick={deletePodcast}>
							Delete Podcast
						</Button>
					</div>
				</Modal.Body>
			...
			<div className='flex flex-col gap-4'>
				{from === 'create' && (
					<div className='self-end'>
						<p className='text-system-primary-accent font-medium cursor-pointer' onClick={() => viewPodcastPage()}>
							Skip for now
						</p>
					</div>
				)}

				{/* <Permissions from={from} discussion={podcast} discussionId={podcastId} permissionType='CanInviteOthers' /> */}
				<Permissions from={from} discussion={podcast} discussionId={podcastId} permissionType='CanPostActivity' />
				{/* <Permissions from={from} discussion={podcast} discussionId={podcastId} permissionType='CanUploadPhoto' /> */}
				{/* <Permissions from={from} discussion={podcast} discussionId={podcastId} permissionType='CanCreateAlbum' /> */}
				<Permissions from={from} discussion={podcast} discussionId={podcastId} permissionType='CanUploadVideo' />
				<Permissions from={from} discussion={podcast} discussionId={podcastId} permissionType='IsAdmin' />
				...
				{from === 'settings' && (
					<div className='border flex flex-col p-4 rounded-md justify-between'>
						<div className='flex flex-col md:flex-row gap-2 justify-between mb-6 md:mb-0 md:items-center'>
							<div>
								<h1 className='text-system-error font-medium text-lg'>Delete the podcast</h1>
								<p className='text-brand-gray mt-1 mb-2 text-base'>
									Deleting podcast will remove all the activity and data in the podcast.
								</p>
							</div>
