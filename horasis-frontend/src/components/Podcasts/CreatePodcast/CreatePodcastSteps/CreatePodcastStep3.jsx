...
const CreatePodcastStep3 = ({ podcastId, from = 'create' }) => {
	...
	const getConnections = (tempConnections) => {
		getData(`discussions/${podcastId}/members/invite?${jsonToQuery(filters)}`, tempConnections, setConnections)
	}
	...
	useEffect(() => {
		if (connections.length > 0) hasAnyLeft(`discussions/${podcastId}/members/invite`, connections)
	}, [connections])
									...
									{isLoading ? (
										...
										<>
											{connections.map((item) => {
												return <InviteMemberTab connection={item} key={item.DocId} podcastId={podcastId} />
											})}
										</>
									)}
								</>
...
export default CreatePodcastStep3
