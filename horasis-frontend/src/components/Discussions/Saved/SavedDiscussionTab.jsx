...
const SavedDiscussionTab = ({ bordered = false }) => {
    ...
    const getDiscussions = (tempActivites) => {
        getData(
            `user/${currentUserData.CurrentUser.UserId}/discussions/save?&${jsonToQuery(filters)}`,
            tempActivites,
            setDiscussions
        )
    }
    ...
    useEffect(() => {
        if (discussions.length > 0)
            hasAnyLeft(`user/${currentUserData.CurrentUser.UserId}/discussions/save`, discussions)
    }, [discussions])
